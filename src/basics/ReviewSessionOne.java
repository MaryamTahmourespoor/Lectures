...
//setup Webdriver   // 1. First step to setup Webdriver to execute a browser we need to setup
...
//create object  	// 2. we need to create reference to Webdriver Interface and then create object
					// of
					// browser choice to execute the test cases.
					WebDriver driver = new ChromeDriver(); // if want to run test case on chrome
					WebDriver driver2 = new FirefoxDriver();// if want to run test case on firefox
					// 3. Setup some windows and time properties

//maximizing		// for maximizing the browser we need to write below codes
...
//lunching a url    // for lunching a url or opening an application we need to write below code ]
					// and this method accepts an argument as String
					driver.get("url");
//get page title	// to get page title we need to write below code
...
//Navigation commands
					...
					driver.quit();// this will close all tabs opened by selenium
//find an element					
...
//click on an element																			// will store it in WebElement interface
...
//enter value to an element										// String
...
// Locators:
					...
					// LinkText:
					// PartialLinkText:
// Xpath: when we don t have a unique ID and Unique Name or any other locators we use
					// Xpath to find the element in UI. we have two type of Xpath.
					
	// 1. Absolute: which starts from top of HTML node and goes all the way down to element tag
...
//find list of elementsssssssssssssssssssssss​
...
//size of a list	// to get size of a list we need to use below method
					elementName.size();
...
//Actions   		// Selenium provides a class name Actions and we can right click, double click
					...
					action.dragAndDrop(element, element).build().perform(); // this will drag an element from source and drop it to
																			// target
//Static DropDowns	// Selenium provides Select class for Static DropDowns that have fixed value
					// and Select Tag.
...
//Browser pops		// Alerts and Browser pops
//handle alerts, and popups	// In selenium we can handle alerts, and popups by using switch method
...
//handle multiple windows	// we need to import java.util.Set
...
//child window		// When child window opens after clicking on a link we need to switch the driver
     				// from parent window to child
					driver.switchTo().window(childWindow);
			
//popups			// Some websites will through Authentication popups
